...
class MySQLInstanceViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['post'], url_path='sync-databases')
    def sync_databases(self, request, pk=None):
        """
        同步实例数据库列表

        POST /instances/{id}/sync-databases/?refresh_stats=1&include_system=0
        """
        instance = self.get_object()
